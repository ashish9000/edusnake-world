...
export default function useGameEngine(canvasRef, mode, onGameOver) {
  ...
  const draw = useCallback(() => {
    ...
    items.forEach(item => {
      ...
      if (isT) {
        // Pulsing glow ring for target
        const pulse = 0.85 + Math.sin(s.tongueTimer * 0.18) * 0.15;
        ctx.shadowColor = accentColor;
        ctx.shadowBlur  = 18 * pulse;

        // Outer ring
        ctx.beginPath();
        ctx.arc(0, 0, cs * 0.48 * pulse, 0, Math.PI * 2);
      ...
      // Item circle
      const grad = ctx.createRadialGradient(-cs*0.1, -cs*0.1, 0, 0, 0, cs*0.42);
      if (isT) {
      ...
      ctx.fillStyle   = grad;
      ctx.fill();
      ctx.strokeStyle = isT ? accentColor : 'rgba(255,255,255,0.2)';
      ctx.lineWidth   = isT ? 2.5 : 1;
      ctx.stroke();

      // Text
      const fontSize = item.value.length > 2 ? cs * 0.27 : cs * 0.38;
      ctx.fillStyle    = isT ? '#fff' : 'rgba(255,255,255,0.8)';
