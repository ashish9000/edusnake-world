// =============================================
// EDUSNAKE WORLD - GameScreen v3 PREMIUM
// =============================================
...
function Confetti({ active }) {
  ...
  return (
    <div style={{ position:'absolute', inset:0, pointerEvents:'none', zIndex:20, overflow:'hidden' }}>
      ...
      <style>{`@keyframes confettiFall{0%{opacity:1;transform:translateY(0) rotate(0deg)}100%{opacity:0;transform:translateY(180px) rotate(720deg)}}`}</style>
    </div>
  );
}
    ...
    <div style={{
      ...
      {wordObj.word.split('').map((ch, i) => (
        <div key={i} style={{
          ...
          border: i < wordIdx ? '2px solid #00C853'
                : i === wordIdx ? '2px solid #FFD700'
                : '2px solid rgba(255,255,255,0.2)',
          background: i < wordIdx ? '#00C85325'
                    : i === wordIdx ? '#FFD70020' : 'transparent',
          display:'flex', alignItems:'center', justifyContent:'center',
          color: i <= wordIdx ? '#fff' : 'rgba(255,255,255,0.3)',
          fontWeight:900, fontSize:13,
          animation: i === wordIdx ? 'wpulse 0.5s infinite alternate' : 'none',
        }}>
          {i < wordIdx ? ch : i === wordIdx ? ch : '·'}
        </div>
      ))}
      <style>{`@keyframes wpulse{from{transform:scale(1)}to{transform:scale(1.12)}}`}</style>
    </div>
  );
}
    ...
    return (
      <button
        ...
        style={{
          gridArea: area,
          background: isP
            ? 'linear-gradient(135deg,#00E676,#00A040)'
          ...
          transform: isP ? 'scale(0.86)' : 'scale(1)',
          transition: 'all 0.08s',
          boxShadow: isP
            ? '0 0 20px #00E67699, inset 0 2px 4px rgba(255,255,255,0.3)'
            : '0 6px 16px rgba(0,0,0,0.4), inset 0 1px 3px rgba(255,255,255,0.2)',
        }}
      >{symbol}</button>
    );
  };
...
// ── Garden/World background themes per mode ───
const MODE_BG = {
  alpha:   { type: 'garden',  colors: ['#0a2e0a','#1a5c1a','#0d3d0d'] },
  hindi:   { type: 'sunset',  colors: ['#2e1400','#8a3d00','#cc6600'] },
  vyanjan: { type: 'candy',   colors: ['#2e0033','#660066','#aa00aa'] },
  number:  { type: 'ocean',   colors: ['#001a33','#003366','#0055aa'] },
  word:    { type: 'space',   colors: ['#050510','#0d0d2e','#1a1a4e'] },
  math:    { type: 'desert',  colors: ['#1a1000','#4d3300','#806600'] },
  shapes:  { type: 'aurora',  colors: ['#001a0d','#003d1a','#006633'] },
  colors:  { type: 'rainbow', colors: ['#1a0033','#330066','#550099'] },
};
...
export default function GameScreen({ mode, onGameOver, onMenu }) {
  ...
  const { start, changeDir, displayState } = useGameEngine(
  ...
  const livesArr = Array.from({ length: 3 }, (_, i) => i < (s?.lives ?? 3));
  ...
  return (
    <div style={{
      ...
      width: '100%', height: '100%',
      background: `linear-gradient(${155 + bgShift}deg, ${modeBg.colors[0]} 0%, ${modeBg.colors[1]} 50%, ${modeBg.colors[2]} 100%)`,
      overflow: 'hidden',
      position: 'relative',
    }}>

      {/* Garden/world decorative elements */}
      <WorldDecor mode={mode} tick={bgTick} />
      ...
      <div style={{
        ...
        borderBottom: `2px solid ${modeData?.color}44`,
        zIndex: 5,
      }}>
        <button
        ...
        <div style={{ display: 'flex', gap: 3 }}>
          {livesArr.map((alive, i) => (
            <span key={i} style={{
              fontSize: 20,
              filter: alive ? 'drop-shadow(0 0 6px #ff4444)' : 'grayscale(1) opacity(0.4)',
              transition: 'all 0.3s',
            }}>❤️</span>
          ))}
        </div>
      ...
      <div style={{ height: 6, background: 'rgba(255,255,255,0.08)', flexShrink: 0 }}>
        <div style={{
          ...
          boxShadow: `0 0 8px ${modeData?.color || '#00c853'}`,
        }} />
      </div>
      ...
      <div style={{
        ...
        <canvas
          ...
          style={{
            display: 'block',
            animation: shakeCanvas ? 'gameShake 0.35s' : 'none',
            borderRadius: 4,
          }}
...
// ── World Decorations per mode ────────────────
function WorldDecor({ mode, tick }) {
  const decorations = {
    alpha: ['🌸','🌿','🍀','🌺','🦋','🌻','🐝','🌱'],
    hindi: ['🪔','✨','🌸','🎆','⭐','🌟','💫','🪷'],
    vyanjan: ['🍭','🎀','🌈','🎠','🎡','🎪','🎭','🎨'],
  ...
  const items = decorations[mode] || decorations.alpha;
  return (
    <div style={{ position:'absolute', inset:0, pointerEvents:'none', overflow:'hidden', zIndex:1 }}>
      {items.map((emoji, i) => {
        const y = Math.sin(tick * 0.015 + i * 0.9) * 6;
        const x = Math.cos(tick * 0.01 + i * 0.7) * 4;
        return (
          <div key={i} style={{
            position:'absolute',
            left: `${5 + i * 11.5}%`,
