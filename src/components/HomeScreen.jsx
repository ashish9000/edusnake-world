// =============================================
// EDUSNAKE WORLD - HomeScreen v2 (Super Colorful!)
// =============================================
import React, { useEffect, useState } from 'react';
import { loadState } from '../utils/storage';
import SFX from '../utils/audio';

const GAME_MODES_ORDERED = [
...
export default function HomeScreen({ onStart, onOpenShop, onOpenProgress }) {
  ...
  useEffect(() => {
    const t = setInterval(() => setBgIdx(i => (i + 1) % BG_THEMES.length), 4000);
    return () => clearInterval(t);
  }, []);
    ...
    <div style={{
      ...
      <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
        {['🌟','⭐','✨','💫','🎈','🎀','🍭','🌈','🦋','🌸'].map((e, i) => (
          <div key={i} style={{
            ...
            fontSize: 16 + (i % 3) * 7,
            opacity: 0.12,
            animation: `floatBubble ${2.5 + i * 0.4}s ease-in-out ${i * 0.3}s infinite alternate`,
          }}>{e}</div>
        ))}
      </div>
      ...
      <div style={{
        ...
        <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: 10 }}>
          EduSnake World v2.0 • swipe or tap arrows
        </div>
      </div>
