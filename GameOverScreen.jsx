// =============================================
// EDUSNAKE WORLD - GameOverScreen Component
// =============================================
...
function StatBox({ label, value, emoji }) {
  return (
    <div style={{
      ...
      <div style={{ color: '#fff', fontSize: 18, fontWeight: 900, fontFamily: "'Fredoka One', cursive" }}>{value}</div>
      <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: 10, fontWeight: 700 }}>{label}</div>
    </div>
  );
}
...
export default function GameOverScreen({ result, onPlayAgain, onMenu }) {
  ...
  const learnedList = (result?.learned || []).slice(0, 12).join('  ');
  ...
  return (
    <div style={{
      ...
      {/* Mascot */}
      <div style={{ fontSize: 60, animation: 'wobble 1s infinite' }}>
        {result?.score > 100 ? '🥳' : '😵'}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, width: '100%', maxWidth: 360 }}>
        <StatBox label="Learned" value={result?.learned?.length ?? 0} emoji="📚" />
        <StatBox label="Words"   value={result?.wordsCompleted ?? 0}  emoji="✍️" />
        <StatBox label="Best"    value={result?.savedState?.highScore ?? 0} emoji="🏆" />
      </div>
      ...
      {learnedList && (
        <div style={{
          ...
          <div style={{ color: '#00c853', fontSize: 11, fontWeight: 800, marginBottom: 4 }}>
            ✅ You Learned:
          </div>
          <div style={{ color: '#fff', fontSize: 14, fontWeight: 700, letterSpacing: 2 }}>
            {learnedList}
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, width: '100%', maxWidth: 300 }}>
        <button
          onClick={() => { SFX.menuClick(); onPlayAgain?.(); }}
          style={{
            fontFamily: "'Fredoka One', cursive",
            fontSize: 20, border: 'none', borderRadius: 18,
