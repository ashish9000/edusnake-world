...
function DPad({ onDir }) {
  const btn = (label, dx, dy) => (
    <button
      onPointerDown={e => { e.preventDefault(); onDir(dx, dy); }}
      style={{
      ...
      onPointerEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.22)'}
  ...
  return (
    <div style={{
      ...
      <div style={{ gridArea: 'up'    }}>{btn('⬆️', 0, -1)}</div>
      <div style={{ gridArea: 'left'  }}>{btn('⬅️', -1, 0)}</div>
      <div style={{ gridArea: 'right' }}>{btn('➡️', 1, 0)}</div>
      <div style={{ gridArea: 'down'  }}>{btn('⬇️', 0, 1)}</div>
    </div>
  );
}
